...
def llm_compare(prompt_data) -> str:
    ...
    text_content = [{'type':'text','text': prompt_data}]
    text_payload = {"messages":[{"role":"user","content":text_content}], "anthropic_version":"bedrock-2023-05-31" ,"max_tokens":1000}
    ...
    response = bedrock.invoke_model(body=json.dumps(text_payload),
                                    modelId=modelId,
                                    accept=accept,
...
def prompt_finder(question):
    ...
    local_embeddings=BedrockEmbeddings(model_id="amazon.titan-embed-text-v2:0",client=bedrock)
...
def doc_compare(uploaded_file_1, uploaded_file_2) -> str:
    ...
    changes but also pinpoints where each change occurs in both documents. Create a summary table of the comparisons between the two Documents.
